...
import {
  ...
  fetchEstados,
  fetchArticulos,
  updateArticulos, // Importa la función para actualizar artículos desde la API
...
/**
 * Componente modal para modificar el stock de artículos.
 *
...
const ModificarStockModal = ({ isOpen, onRequestClose }) => {
  ...
  // Estado para manejar la búsqueda de artículos
  const [searchTerm, setSearchTerm] = useState('');
  const [loading, setLoading] = useState(false);

  // Estados para manejar la edición de artículos
  const [isEditing, setIsEditing] = useState(false);
  const [selectedArticulo, setSelectedArticulo] = useState(null);

  // Estados para los campos editados del artículo
  const [editedFields, setEditedFields] = useState({
    ...
    ubicacion: '',
    stock_actual: '',
    // "estado" fue removido para que no se edite aquí
  });

  // Estados para manejar errores de duplicación
  ...
  // Indicador de procesamiento durante operaciones de edición
    ...
    if (isOpen) {
      const cargarDatos = async () => {
        ...
        try {
          ...
          // Mapea los datos para el componente react-select
          setCategorias(
  ...
  /**
   * Maneja los cambios en los campos de texto del formulario de edición.
   *
  ...
  /**
   * Maneja los cambios en los campos de selección del formulario de edición.
   *
   * @param {object} selectedOption - Opción seleccionada.
   * @param {object} actionMeta - Metadatos de la acción.
   */
  ...
  /**
   * Filtra los artículos basándose en el término de búsqueda ingresado.
   *
   * @param {array} list - Lista de artículos.
   * @param {string} term - Término de búsqueda.
   * @returns {array} Lista filtrada de artículos.
   */
  const filterArticulos = useCallback(
    (list, term) => {
      ...
      return list.filter((art) => {
        ...
        const ubiLabel = ubicaciones.find((ub) => ub.value === art.ubicacion)?.label || '';
        const estLabel = estados.find((es) => es.value === art.estado)?.label || '';
        ...
        return (
          ...
          ubiLabel.toLowerCase().includes(lowerTerm) ||
          estLabel.toLowerCase().includes(lowerTerm) ||
   ...
   *
   * @param {object} art - El artículo a editar.
   */
  ...
  /**
   * Guarda los cambios realizados en el artículo editado.
   */
  const handleSave = async () => {
    // Validación: Si la categoría es 'PC', el campo MAC es obligatorio
    ...
    // Verificar duplicados en campos específicos
    ...
    // Construir el payload con los campos actualizados
    ...
    // Si la categoría no es 'PC', remover el campo MAC
    ...
    // Llamar a la API para actualizar el artículo
    try {
      ...
      // Actualizar localmente el artículo editado
      setArticulos((prev) =>
        prev.map((a) =>
  ...
  /**
   * Cancela la edición del artículo y resetea los estados relacionados.
   */
  const handleCancel = () => {
    setIsEditing(false);
    setSelectedArticulo(null);
  ...
  return (
    <>
      {/* Modal Principal: Lista de Artículos */}
      <Modal
        isOpen={isOpen}
      ...
      {/* Segundo Modal: Edición de Artículo */}
      {isEditing && selectedArticulo && (
        <Modal
          isOpen={isEditing}
          onRequestClose={handleCancel}
          contentLabel="Editar Artículo"
          className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-auto p-6 overflow-auto relative
        ...
        >
          ...
          <form className="space-y-6">
            ...
            {/* Fila 3: Código MINVU, Código Interno, Número de Serie, MAC */}
